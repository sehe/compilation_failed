...
namespace parse {
    ...
    auto const declaration_specifier_def = storage_class_specifier //
        | type_specifier                                           //
        | type_qualifier;
    auto const storage_class_specifier_def = //
        x3::string("auto")                   //
        | x3::string("register")             //
        | x3::string("static")               //
        | x3::string("extern")               //
    ...
    auto const struct_or_union_specifier_def =
    ...
    auto const struct_declarator_list_def =
    ...
    auto const direct_declarator_def = (identifier | ('(' >> declarator >> ')')) //
        >> *((x3::lit('[') >> -conditional_expression >> ']')                    //
             | (x3::lit('(') >> parameter_type_list >> ')')                      //
             | (x3::lit('(') >> *identifier >> ')'))                             //
        ;
    ...
    auto const logical_or_expression_def = logical_and_expression >>
        *(x3::lit("||") >> logical_and_expression);
    ...
    auto const equality_expression_def     = relational_expression >>
        *(equality_expression_oper >> relational_expression);
    ...
    auto const additive_expression_def = multiplicative_expression >>
        *(additive_oper >> multiplicative_expression);
    ...
    auto const unary_expression_def = *unary_expression_oper >>
        ((unary_operator >> cast_expression) //
         | (x3::lit("sizeof") >> type_name) | postfix_expression);
    // auto const unary_expression_def = *unary_expression_oper >> (unary_operator >> cast_expression);

    // custom change
    // auto const postfix_expression_def = primary_expression >> *(('[' >> expression >> ']') | ('(' >>
    ...
    auto const assignment_operator_def = //
        x3::string("*=")                 //
        | x3::string("/=")               //
        | x3::string("%=")               //
        | x3::string("+=")               //
    ...
    // auto const direct_abstract_declarator_def = (x3::lit('(') >> abstract_declarator >> ')') >>
    // *((x3::lit('[') >> conditional_expression  >> x3::lit(']')) | (x3::lit('(') >> parameter_type_list  >>
    // x3::lit(')')) | (x3::lit('(') >> ')') | (x3::lit('[') >> ']'));
    auto const direct_abstract_declarator_def = (x3::lit('(') >> abstract_declarator >> ')') >>
        *((x3::lit('[') >> -conditional_expression >> x3::lit(']')) //
          | (x3::lit('(') >> -parameter_type_list >> x3::lit(')'))  //
        );

    auto const enum_specifier_def  = x3::lit("enum") >> ((identifier >> '{' >> enumerator_list >> '}') //
                                                        | ('{' >> enumerator_list >> '}')             //
                                                        | identifier);
    ...
    auto const selection_statement_def =
        (x3::lit("if") >> '(' >> expression >> ')' >> ((statement >> "else" >> statement) | statement)) //
    ...
    auto const iteration_statement_def = (x3::lit("while") >> '(' >> expression >> ')' >> statement) //
        ...
        | (x3::lit("for") >> '(' >> -expression >> ';' >> -expression >> ';' >> -expression >> ')' >>
           statement);

    // auto const jump_statement_def = ((x3::lit("goto") >>  identifier) | "continue" | "break" | "return" |
    // ("return" >> expression) ) >> ';';
    ...
    BOOST_SPIRIT_DEFINE(translation_unit, external_declaration, function_definition,
                        declaration_specifier /*,storage_class_specifier*/, type_specifier);

    BOOST_SPIRIT_DEFINE(struct_or_union_specifier /*,struct_or_union*/, struct_declaration,
                        specifier_qualifier, struct_declarator_list, struct_declarator, declarator, pointer)
    BOOST_SPIRIT_DEFINE(/*type_qualifier,*/ direct_declarator, conditional_expression, logical_or_expression,
                        logical_and_expression)
    BOOST_SPIRIT_DEFINE(inclusive_or_expression, exclusive_or_expression, and_expression, equality_expression,
                        relational_expression, shift_expression)
    BOOST_SPIRIT_DEFINE(additive_expression, multiplicative_expression, cast_expression, unary_expression,
                        postfix_expression, primary_expression, constant)
    BOOST_SPIRIT_DEFINE(expression, assignment_expression /*,assignment_operator,unary_operator*/, type_name,
                        parameter_type_list, parameter_list, parameter_declaration)
    BOOST_SPIRIT_DEFINE(abstract_declarator, direct_abstract_declarator, enum_specifier, enumerator_list,
                        enumerator, typedef_name, declaration, init_declarator)
    BOOST_SPIRIT_DEFINE(initializer, initializer_list, compound_statement, statement, labeled_statement,
